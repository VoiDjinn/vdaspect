...
void VDAspectComposer::_bind_methods()
{
  ...
  ClassDB::bind_method(D_METHOD( "add_aspect",    "aspect" ), &VDAspectComposer::add);
  ClassDB::bind_method(D_METHOD( "remove_aspect", "aspect" ), &VDAspectComposer::remove);

  ClassDB::bind_method(D_METHOD( "has_aspect", "name" ), &VDAspectComposer::has);
  ClassDB::bind_method(D_METHOD( "get_aspect", "name" ), &VDAspectComposer::get);
...
void VDAspectComposer::reg(Ref<VDAspectNode> node)
{
  ...
  {
    reg(node->get_child(i));
  }
}

void VDAspectComposer::unreg(Ref<VDAspectNode> node)
{
  List<StringName> classes = node->get_classes();
  for (List<StringName>::Element* ele = classes.front(); ele; ele = ele->next())
  {
  ...
  for (int i = 0; i < node->get_child_size(); i++)
  {
    unreg(node->get_child(i));
  }
}


bool VDAspectComposer::add(Ref<VDAspect> aspect)
{
  if (!roots.has(aspect))
  {
...
bool VDAspectComposer::remove(Ref<VDAspect> aspect)
{
...
bool VDAspectComposer::has(StringName name)
{
  return aspects.has(name);
}

Ref<VDAspect> VDAspectComposer::get(StringName name)
{
